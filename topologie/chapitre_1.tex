...
$\mathbf{RP}^0 \cong \quot{\{\pm 1\}}{-1 \sim 1}$, $\mathbf{RP}^1 \cong S^1$, $\mathbf{RP}^2 \cong \quot{D^2}{\sim}$
	...
	\begin{remark}
		...
		\begin{minipage}{0.5\textwidth}
			\begin{align*}
				&i : X \xmapsto{i_1} X \sqcup Y \xmapsto{q} X \cup_A Y \\
				& j : Y \xmapsto{i_2} X \sqcup Y \xmapsto{q} X \cup_A Y			
			\end{align*}
		\end{minipage}
		\hfill
		\begin{minipage}{0.5\textwidth}
	...
	\begin{prop}[Propriété universelle]
		...
		\begin{minipage}{0.5\textwidth}
			...
			\xymatrix@R=1.2cm@C=1.2cm{
			 ...
			 X \ar[r]^{i} \ar@/_1pc/[rrd]_{\alpha} & X\cup_A Y \ar@{-->}[rd]|{\exists! \; \gamma}             \\
							      &                                   & Z 
			}
	\]	
		\end{minipage}
